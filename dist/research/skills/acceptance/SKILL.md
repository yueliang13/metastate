...
and acceptance criteria for the **research** domain.
...
Research findings must be verified with at least 2 independent primary sources. Methodology must be reproducible. Claims must cite primary sources, not secondary summaries.

### Accept/Reject Labels

- Accept (task passes): **VERIFIED**
- Reject (task needs rework): **UNVERIFIED**
...
- What the status line must show (VERIFIED vs
  UNVERIFIED)
...
The verification mode for this domain is: **semi-auto**
...
is UNVERIFIED regardless of any other qualities.
...
- How this interacts with the issue lifecycle — Pending issues mean
  UNVERIFIED, Resolved/Don't Fix issues may still
  be VERIFIED if the agent addressed them
...
VERIFIED — [brief description of what this means]

## Unacceptable Status

UNVERIFIED — [brief description of what this means]
```
...
5. The accept/reject decision section must make it crystal clear how the
   ...
   for interpretation.
6. Use the exact labels VERIFIED and
   UNVERIFIED throughout — do not substitute
