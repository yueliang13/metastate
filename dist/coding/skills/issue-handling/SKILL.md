...
**在进行各项工作的过程中, 必须充分考虑所有已知问题的已有假设(Assumption). 因此:**
  ...
  - 编写或review相关内容之前, 要先读spec问题列表.

  - 编写或review相关内容之前, 要先读plan问题列表.
...
在编写任务文档(task.md)的时候, 需要遵循spec.md文档, 如果文档中有模糊不清或
前后矛盾的地方, 那么agent就必须自己做出合理假设来解决并记录问题.

**注意:**
  - 做任务规划时违反spec.md所引起的问题不是spec问题, 必须马上解决, 禁止记录下来不处理
  - 编写spec.md时发现的spec问题必须马上解决, 禁止记录下来不处理
...
## SI-001: [title]
- **Description**: [Spec ambiguity or contradiction]
...
在编写代码和测试的时候, 需要遵循任务文档(task.md)中的任务描述, 如果任务描述中有模糊不
清或前后矛盾的地方, 那么agent就必须自己做出合理假设来解决并记录问题.

**注意:**
  - 实现者违反任务描述所引起的问题不是plan问题, 必须马上解决, 禁止记录下来不处理
  - 做任务规划和编写任务文档时发现的plan问题, 必须马上解决, 禁止记录下来不处理
  - 已有代码中的bug就算任务中没有提到也不属于其问题, 必须主动解决bug, 禁止记录下来不处理


路径: working/plan-issues.md
记录格式:
