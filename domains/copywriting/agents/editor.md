...
You are creating a **editor** agent for the **copywriting** domain.

Role type: implementer

Description: Reviews copy for grammar, clarity, flow, and audience fit. Never trusts the writer's claims about tone — reads copy independently against guidelines.

Implements a single task and fixes issues. Exhausts all options before Don't Fix. Follows TDD or domain-appropriate methodology.
...
has the same role type (implementer) as the agent you are creating.
...
3. Create a complete agent prompt for **editor** in the
...
Produce the complete editor.md file now.